    ...
    for (int i = 0; i < NumVarInfos; i++) {
        ...
        for (const auto &range : *var.ranges) {
            ...
            case ICorDebugInfo::VLT_REG_FP: {
                ...
                OST.EmitIntValue(reclen, 2);
                OST.EmitIntValue(rectyp, 2);
                OST.EmitBytes(StringRef((char *)&rec, offsetof(DefRangeRegisterSym, Range)));
                break;
            }
            ...
            case ICorDebugInfo::VLT_STK: {
                ...
                OST.EmitIntValue(reclen, 2);
                OST.EmitIntValue(rectyp, 2);
                OST.EmitBytes(StringRef((char *)&rec, offsetof(DefRangeRegisterRelSym, Range)));
                break;
            }
...
extern "C" void EmitDebugVar(ObjectWriter *OW, char *name,
    int typeIndex, bool isParm, int rangeCount,
    char *ranges) {
    assert(OW && "ObjWriter is null");
