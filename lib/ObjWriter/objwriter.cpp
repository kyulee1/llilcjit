...
extern "C" void EmitCFIBlob(ObjectWriter *OW, int Offset, const char *Blob) {
  assert(OW && "ObjWriter is null");
  assert(OW->frameOpened && "frame should be opened before CFI emission");
  auto *AsmPrinter = &OW->getAsmPrinter();
  auto &OST = *AsmPrinter->OutStreamer;
